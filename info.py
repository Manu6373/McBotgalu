...
# FORCE_SUB 
auth_req_channel = environ.get('AUTH_REQ_CHANNEL', '-1002653255036')  # requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
AUTH_CHANNELS = [int(channels_id) for channels_id in environ.get('AUTH_CHANNELS', '-1002653255036').split() if re.match(r'^-?\d+$', channels_id)]  # Channels for force sub (make sure bot is admin)
